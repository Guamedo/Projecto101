...
public:
    ...
    Player(string nombreEntidad, int x, int y, int tipoo, int diam) : Entity(nombreEntidad, x, y, tipoo) {
        diametro = diam;
    }
};
...
Player player = Player("player", 25, 25, 1, 0);
...
double eucDist(int x1, int y1, int x2, int y2) {
...
void drawPlayer() {
    ...
    int x, y;
    for (int i = -player.diametro; i <= player.diametro; i++) {
        for (int j = -player.diametro; j <= player.diametro; j++) {
            x = player.posx - i;
            y = player.posy - j;
            if (eucDist(x, y, player.posx, player.posy) <= player.diametro) {
                glVertex2d(x, y);
            }
        }
    }
